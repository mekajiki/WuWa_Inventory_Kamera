...
## 注意: 合計ステータス(stats)の汚染

characters[id].stats はステータス画面の実測値なので、**編成中の他キャラの
常時パッシブ(緋雪の厳霜系など)や食事バフが乗り得る**。音骸・スキル・凸・
武器の読み取りは編成の影響を受けない。stats を厳密に使う場合は中立編成
(バフ持ちを外す/単騎)でスキャンするか、部品からの理論合計との残差で
汚染をフラグする(育成計画側の処理)。
